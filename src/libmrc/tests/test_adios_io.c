...
typedef void (*get_size_t)(struct mrc_io *, uint64_t *size);
typedef void (*set_size_t)(struct mrc_io *, uint64_t size);
...
main(int argc, char **argv)
{
  ...
  const char *adios_steps[] = { "adios_define", "adios_size", "adios"};
  ...
  for (int step = 0; step < 3; step++) {
    ...
    mrc_io_set_name(io, "field_test");
    mrc_io_set_param_string(io, "outdir", "adios_test");
    mrc_io_setup(io);

    if (step == 2) {
      set_size_t set_payload_size = (set_size_t) mrc_io_get_method(io, "set_group_size");
    ...
    if (step == 1) {
      get_size_t get_payload_size = (get_size_t) mrc_io_get_method(io, "final_size");
  ...
  ierr = adios_finalize(rank); assert(ierr == 0);
  //mrc_io_view(io);
